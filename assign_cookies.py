...
def findContentChildren(greed_factors, cookie_sizes):
    sorted_greed_factors = sorted(greed_factors, reverse=True)
    sorted_cookie_sizes = sorted(cookie_sizes, reverse=True)
    index_of_cookie_sizes = 0
    for greed_factor in sorted_greed_factors:
        if index_of_cookie_sizes >= len(cookie_sizes):
            return index_of_cookie_sizes
        if sorted_cookie_sizes[index_of_cookie_sizes] >= greed_factor:
            index_of_cookie_sizes += 1
    return index_of_cookie_sizes
